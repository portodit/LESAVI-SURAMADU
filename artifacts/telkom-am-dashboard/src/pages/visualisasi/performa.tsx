...
import {
  ...
  ChevronRight, Camera, ChevronUp, Star, Expand, Minimize2
} from "lucide-react";
...
export default function PerformaVis() {
  ...
  const [filterNamaAm, setFilterNamaAm] = useState("All");
  ...
  const amTableData = useMemo(() => {
    ...
    if (filterNamaAm !== "All") result = result.filter(r => r.namaAm === filterNamaAm);
  ...
  }, [allPerfs, filterYear, filterMonths, cmMonth, filterSnapshotId, filterDivisi, filterNamaAm, filterTipeRank, filterTipeRevenue]);
  ...
  React.useEffect(() => { if (filterNamaAm !== "All" && !amNames.includes(filterNamaAm)) setFilterNamaAm("All"); }, [amNames]);
      ...
      <div className="bg-card border border-border rounded-xl p-4">
        <div className="flex flex-wrap gap-3 items-end">
              ...
              {MONTHS_LABEL.map((label, idx) => {
                ...
                return (
                  <button
                    ...
                    disabled={!hasData}
                    className={cn(
                      "h-8 px-2.5 text-xs font-semibold rounded-lg border transition-colors",
                      hasData
                        ? checked
          ...
          <div className="flex flex-col gap-1 min-w-[140px]">
            <label className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide">Nama AM</label>
            <select
              value={filterNamaAm}
              onChange={e => setFilterNamaAm(e.target.value)}
              disabled={!amNames.length}
              className="h-8 px-2.5 bg-secondary/50 border border-border rounded-lg text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary disabled:opacity-40"
            >
              <option value="All">Semua AM</option>
