...
export default function ImportData() {
  ...
  const handleDriveList = async (type: string) => {
    ...
    try {
      ...
      if (files.length > 0) {
        const detected = window.extractDateFromFilename(files[0].name);
        if (detected) {
  ...
  const handleDriveSyncBatch = async (type: string, filesToSync: any[]) => {
    ...
    for (let i = 0; i < filesToSync.length; i++) {
      ...
      try {
        const fDetected = window.extractDateFromFilename(f.name);
        const result = await apiFetch<any>(`/api/gdrive/sync?type=${type}`, {
  ...
  const fileDetected = currentFile ? window.extractDateFromFilename(currentFile.name) : null;
  ...
  const commitFile = useCallback((file: File, sheet: string) => {
    ...
    const detected = window.extractDateFromFilename(file.name);
    if (detected) {
    ...
    <div className="space-y-5">
      ...
      <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm">
        ...
        {activeTab !== "target-ho" && activeTab !== "target-am" && (() => {
          ...
          const driveDetected = driveFilesList.length > 0 ? window.extractDateFromFilename(driveFilesList[0].name) : null;
